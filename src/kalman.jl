...
"""
function KalmanFilter(A,B,C,D,R1,R2,d0=SimpleMvNormal(Matrix(R1)); Ts = 1, p = NullParameters(), α = 1.0, check = true, nx = length(d0), ny = size(C,1), nu = size(B,2), names = default_names(length(d0), nu, ny, "KF"))
    if check
