...
- [`SqExtendedKalmanFilter`](@ref): A square-root version of the EKF.
...
- [`EnsembleKalmanFilter`](@ref): The Ensemble Kalman filter uses an ensemble of states instead of explicitly tracking the covariance matrix. This makes it suitable for high-dimensional systems where covariance matrices become intractable. Uses the stochastic EnKF formulation with perturbed observations. Like the UKF, it handles nonlinear dynamics but assumes Gaussian posteriors.
...
### Experimental estimator types
Additionally, these filter types are currently considered experimental and may change in future releases without respecting semantic versioning.
